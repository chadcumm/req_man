...
012   10/08/2021  Chad Cummings			CST-141739 changed i2 variable to i4
******************************************************************************/
DROP PROGRAM 2req_cust_mp_req_by_loc_dt GO
CREATE PROGRAM 2req_cust_mp_req_by_loc_dt
 prompt 
...
%i cust_script:temp_play_routines.inc
...
;		and   parser(document_date_parser)
	...
	*/
	...
	head o.order_id
		...
		record_data->tlist[d1.seq].ordering_provider_id = p.person_id
		record_data->tlist[d1.seq].ordering_position = uar_get_code_display(p.position_cd)
...
foot parent_event_id
 if (valid_ind = 1)
	...
	output_data->qual[output_data->cnt].ordering_position	= record_data->tlist[d1.seq].ordering_position
	output_data->qual[output_data->cnt].ordering_provider_id	= record_data->tlist[d1.seq].ordering_provider_id
	output_data->qual[output_data->cnt].req_status_dt		= format(record_data->tlist[d1.seq].latest_status_dt_tm,"dd-mmm-yyyy;;q")
	output_data->qual[output_data->cnt].comment				= trim(record_data->tlist[d1.seq].latest_comment)
