...
drop program bc_all_mp_get_pdf_audit:dba go
create program bc_all_mp_get_pdf_audit:dba
prompt
...
;call echo(build2("personId ",$personId)) 
;call echo(build2("encntrId ",$encntrId)) 

declare pencntr_id = f8 with noconstant($encntrId)
...
;call echo(build2("pencntr_id=",pencntr_id))
...
head report
	i =0
	;call echo(build2("inside main query"))
detail
	i = (i+1)
	t_rec->cnt = i
...
;call echo(build2("ending main query, curqual=",curqual))
...
declare loc_pos = i2 with noconstant(0), protect
...
;call echo(build2("starting second query="),t_rec->cnt)
select into "nl:"
from
