...
******************************************************************************
  ...
  Solution:           
  Source file name:   bc_eks_combine_reqs.prg
  Object name:        bc_eks_combine_reqs
  Request #:
...
******************************************************************************/
drop program bc_eks_combine_reqs:dba go
create program bc_eks_combine_reqs:dba
...
for (_i=1 to t_rec->req_cnt)
	if ((t_rec->req_qual[_i].regenerate_ind = 1) and (t_rec->regenerate_cnt >= 0))
		...
		set req_requestin->actionpersonnelid		= 1
		set _j = 0
		if (t_rec->req_qual[_i].ord_cnt > 0)			
		 for (_k=1 to 1);t_rec->req_qual[_i].ord_cnt)
			if (t_rec->req_qual[_i].ord_qual[_k].requisition_format_cd > 0.0)
				...
				set req_requestin->request->orderlist[_j].actiontypecd				= value(uar_get_code_by("MEANING",6003,"MODIFY")) 
		...
		execute pfmt_bc_s_print_to_pdf_req with replace("REQUESTIN",REQ_REQUESTIN)
		execute pfmt_bc_print_to_pdf_req with replace("REQUESTIN",REQ_REQUESTIN)
