...
003   06/30/2021  Chad cummings			Added RM check to control statuses
004   07/07/2021  Chad cummings			Changed RM automation to SYSTEM as the user
******************************************************************************/
drop program dev_all_mp_add_print_status:dba go
create program dev_all_mp_add_print_status:dba
...
with OUTDEV, EVENT_ID, ACTION, APPLICATION
...
(
	...
	1 encntr_id = f8 ;003
	1 loc_unit_cd = f8 ;003
	1 rm_exclude_ind = i2 ;003
	1 req_status = vc ;003
	1 application = vc ;003
) with protect
...
detail
	...
	resultdata->encntr_id = ce.encntr_id
with nocounter
...
for (i = 1 to resultdata->event_cnt)
	...
	set t_rec->application = $APPLICATION
	/*start 003 */
	select into "nl:"
	from 
		encounter e
	plan e
		where e.encntr_id = resultdata->encntr_id
	detail
		t_rec->loc_unit_cd = e.loc_nurse_unit_cd
	with nocounter
	...
	plan cv1
		where cv1.code_value = t_rec->loc_unit_cd
	join cv2
		where cv2.display = cv1.display
		and   cv2.code_set = 103507
		and   cv2.active_ind = 1
	join cve1
		where cve1.code_value = cv2.code_value
		and   cve1.field_name = "RM_EXCLUDE"
	detail
		t_rec->rm_exclude_ind = cnvtint(cve1.field_value)
	with nocounter
	/*end 003*/
...
%i cclsource:status_block.inc
	...
	if ((t_rec->rm_exclude_ind = 1) and (t_rec->application = "IPPDF"))
		call echo(build2("encounter location excluded from rm, updating status"))
		if (t_rec->action =  "REMOVE")
			set t_rec->req_status = ^To Be Actioned^
		else
			set t_rec->req_status = ^Complete^
		endif
		...
		set ensure_request->req[1].event_prsnl.action_dt_tm 	= cnvtdatetime(curdate,curtime3) 
		set ensure_request->req[1].event_prsnl.updt_dt_tm 		= cnvtdatetime(curdate,curtime3)
		set ensure_request->req[1].event_prsnl.action_prsnl_id 	= 1.0 ;004 t_rec->request_prsnl_id
		set ensure_request->req[1].event_prsnl.proxy_prsnl_id 	= 0.00 
		set ensure_request->req[1].event_prsnl.action_status_cd = value(uar_get_code_by("MEANING",103,"COMPLETED"))
		set ensure_request->req[1].event_prsnl.defeat_succn_ind = 1 
