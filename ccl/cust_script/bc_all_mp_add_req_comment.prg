...
******************************************************************************
  ...
  Solution:           
  Source file name:   bc_all_mp_add_req_comment.prg
  Object name:        bc_all_mp_add_req_comment
  Request #:
...
002   01/04/2021  Chad Cummings			Allow empty comment
******************************************************************************/
drop program bc_all_mp_add_req_comment:dba go
create program bc_all_mp_add_req_comment:dba
...
set t_rec->comment = $COMMENT

set record_data->parent_event_id = t_rec->event_id
set record_data->comment = t_rec->comment
				...
				if (cnvtupper(1120120reply->rb_list[k].event_list[j].event_class_cd_disp) = "MDOC")
					...
					detail
						...
						if (cnt = 1)
							...
							t_rec->latest_comment = 1120120reply->rb_list[k].event_list[j].event_prsnl_list[d1.seq].action_comment
						endif
...
if (t_rec->comment = t_rec->latest_comment)
	SET record_data->status_data.status = "S"
	go to exit_script
endif
...
set ensure_request->req[1].event_prsnl.action_type_cd 	= value(uar_get_code_by("MEANING",21,"ASSIST"))  
...
set ensure_request->req[1].event_prsnl.action_comment 	= t_rec->comment
