...
set t_rec->log_filename_request 	= concat ("cclscratch:requestin_560200_" ,t_rec->sysdate_string ,".dat" )
set t_rec->log_filename_a 			= concat ("cclscratch:lab_560200_records_" ,t_rec->sysdate_string ,".dat" )
...
;030
if (requestin->trigger_app = 999999)
...
for (i=1 to size(requestin->orderlist,5))
	...
	set t_rec->trigger_app = requestin->trigger_app
	...
	if (requestin->orderlist[i].ACTIONTYPECD in(
		...
		if (requestin->orderlist[i].REQUISITIONFORMATCD = 0.0)
			call writeLog(build2("---->requisition code empty in request, gathering from database"))
			select into "nl:" from order_catalog oc where oc.catalog_cd = requestin->orderlist[i].CATALOGCD
			detail requestin->orderlist[i].REQUISITIONFORMATCD = oc.requisition_format_cd
			with nocounter
		endif
		...
		if (requestin->CONTRIBUTORSYSTEMCD = uar_get_code_by("DISPLAY",89,"LIS_SYS"))
			set pass_ind = 0
		...
		if (pass_ind = 1)
			call writeLog(build2("----->inside pass_ind = 1"))
				...
				set t_rec->temp_orderlist[t_rec->temp_orderlist_cnt].catalog_cd = requestin->orderlist[i].catalogcd
				set t_rec->temp_orderlist[t_rec->temp_orderlist_cnt].order_mnemonic 
					= uar_get_code_display(requestin->orderlist[i].catalogcd)
				set t_rec->temp_orderlist[t_rec->temp_orderlist_cnt].ordering_provider_id=requestin->orderlist[i].orderproviderid		;029
				set t_rec->temp_orderlist[t_rec->temp_orderlist_cnt].action_personnel_id = requestin->actionpersonnelid					;029
