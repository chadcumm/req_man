...
******************************************************************************/
drop program rm_all_mp_pdf_viewer go
create program rm_all_mp_pdf_viewer 
				...
				if (cnvtupper(1120120reply->rb_list[i].event_list[j].event_class_cd_disp) = "MDOC")
					...
					plan d1
						...
						where 1120120reply->rb_list[i].event_list[j].event_prsnl_list[d1.seq].action_type_cd_disp
								in("Confirm","Order","Modify","Cancel")
						/* end 003 */
...
set 3011001Request->Module_Name = "rm_print_to_pdf_new.html"
