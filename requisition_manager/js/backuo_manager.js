...
function RenderPWxFrame() {
	...
	function generateFilterSets()	{
		...
		pwxheadHTML.push('<input type=text class="pwx_fcr_input_new_set_dt" id="pwx_new_task_filter_set" placeholder="Enter New Filter Name" maxlength="27"></input>');
		...
		var filterSetsHTML = [];
		filterSetsHTML.push('');
		filterSetsHTML.push('<span style="vertical-align:30%;color:red">','Filter(s)',': </span><select id="task_filterset" name="task_filterset" class="pwx_task_filterset_box">');
		filter_set_name = "";
		...
		*/
		...
		$('#pwx_new_filter_set').on('click', function (event) {
			...
			var new_filter_set = document.getElementById("pwx_new_task_filter_set").value;
			...
			var filter_params = [array_of_task_status_values, 
								 ...
								 value_of_requestedreportrange].join("|")
	...
	pwxfilterbarHTML.push('<dt id="pwx_task_pagingbar_printall" class="pwx_grey"></dt>');
	pwxfilterbarHTML.push('<dt id="pwx_task_list_refresh_data" class="pwx_filter_refresh_data"></dt>');	
	pwxfilterbarHTML.push('<div>');
	pwxfilterbarHTML.push('<dt id="pwx_task_filterbar_page_next" class="pwx_task_pagingbar_page_icons"></dt>');
	pwxfilterbarHTML.push('<dt id="pwx_task_filterbar_page_prev" class="pwx_task_pagingbar_page_icons"></dt>');
	pwxfilterbarHTML.push('<dt id="pwx_task_pagingbar_cur_page" class=""></dt>');
...
function RenderTaskListContent(pwxdata) {
	...
	pwxcontentHTML.push('<input type=checkbox name="check_all_requisitions" onclick="toggle(this);">');
	pwxcontentHTML.push('');
	...
	$('.pwx_fcr_input_clerkcomment_dt').on('focusout', function () {
		...
		if (new_comment_length > 0) {
			...
			var temp_comment = doc.body.textContent
	...
	$('#pwx_task_pagingbar_printall').on('click', function () {
		...
		if (checkboxes.length > 10) {
			alert("Please select 10 or fewer requisitions to print at one time");
		} else {
			if (checkboxes.length > 0) {
				...
				task_detailText.push('<td width=25% align=right><div id="printbutton"><button id="print_all" class="myButton">Print '+array.length+' Requisition(s)</button></div></div>');
				task_detailText.push('<td width=50% align=center><div id="printing_status"></div></td></div>');
				...
				for (let i = 0; i < array.length; i++) {
					...
					PWX_CCL_Request("bc_all_mp_pdf_url", sendArr, false, function () {
						task_detailText.push('<div id="PDFDivURL-'+array[i][0]+'" style="display:none">'+this.CMV_URL+'</div>');
						task_detailText.push('<div id="PDFDiv"></div>');
						task_detailText.push('<div id="embPDFDiv-'+array[i][0]+'"></div>');
				...
				$('#open_separate_multi').on('click', function () {
					...
					if (array.length > 0) {
						...
						fwObj.SetPopupStringProp("REPORT_NAME","rm_all_mp_multi_pdf_viewer");
				...
				$('#print_all').on('click', function (event) {
							for (let i = 0; i < array.length; i++) {
								embedPDF(urls[i],array[i][0]);
							}
							var checkboxes = document.querySelectorAll('input[name=print_requisition]:checked')
							if (checkboxes.length > 0) {
								array = []
								...
								for (let i = 0; i < array.length; i++) {
									var documentId = "pdfDocument-" + array[i][0]
									var json_idx = array[i][0]
									var event_id = array[i][1];
									
									p.innerHTML = "Printing " + (i+1) + " of " + (array.length) + " requistions";
								...
								p.innerHTML = "Printed " + (array.length) + " of " + (array.length) + " requistions";
				...
				function embedPDF(PDFurl,index)	{
					var PDFid = document.getElementById("embPDFDiv-" + index);
					var pdfEmbed = document.createElement('embed');
					pdfEmbed.setAttribute('src', PDFurl);
					pdfEmbed.setAttribute('type', 'application/pdf');
					pdfEmbed.setAttribute('width', '0%');
					pdfEmbed.setAttribute('height', '0');
					pdfEmbed.setAttribute('id', 'pdfDocument-'+index);
					pdfEmbed.setAttribute('class', 'pdfDocument');
					pdfEmbed.setAttribute('style', 'display:hidden');
	...
	framecontentElem.on('click', 'span.pwx_fcr_content_type_name_dt a', function () {
		...
		task_detailText.push('<span class="pwx_modal_person_banner_details">Event ID:&nbsp;', pwxdata.TLIST[json_index].PARENT_EVENT_ID, '</span>');
		task_detailText.push('</div></br>');
		task_detailText.push('<table>');
		task_detailText.push('<tr>');
		task_detailText.push('<td width=25% align=right><div id="printbutton"><button id="print_all" class="myButton">Print Requisition</button></div>');
		task_detailText.push('<td width=50% align=center><div id="printing_status"></div></td></div>');
		task_detailText.push('<td width=25% align=right><div id="printing_reference"></div></td></div>');
		task_detailText.push('</tr>');
		task_detailText.push('</table>');
		...
		PWX_CCL_Request("bc_all_mp_pdf_url", sendArr, false, function () {
			...
			task_detailText.push('<div id="PDFDiv"></div>');
			task_detailText.push('<div id="embPDFDiv"></div>');
			task_detailText.push('<div id="embPDFImg"></div>')
		});

		
		task_detailText.push('<br><br>');
		...
		PWX_CCL_Request("req_cust_mp_get_comment_hist", sendArr, false, function () {
		...
		$('#print_all').on('click', function (event) {
			var url = document.getElementById('PDFDivURL').innerHTML;
			var PDFurl = url+'#toolbar=0&navpanes=0&scrollbar=1&view=FitH,top'
			//download(document.documentElement.outerHTML,"debug.html","text/html")
			//prompt("url",url)
            var camm_store_url = 'http://phsacdeanp.cerncd.com/camm/b0783.phsa_cd.cerncd.com/service/PDF_REQUISITION/store'
			var camm_get_url = 'http://phsacdeanp.cerncd.com/camm/b0783.phsa_cd.cerncd.com/service/mediaContent/'
			var camm_attrib_url = 'http://phsacdeanp.cerncd.com/camm/b0783.phsa_cd.cerncd.com/service/mediaAttributes/'
			
			//var downloaded_pdf = this.responseText
			
			var a = new XMLHttpRequest;
			a.open("GET",url,true);
			a.onreadystatechange = function() {
					//alert('requestAsync='+this.status+"readyState="+this.readyState)
				if (this.readyState == 4 && this.status == 200) {
					...
					//doc.addPage()
			//var downloaded_pdf = doc.output('pdfobjectnewwindow');
			...
			doc.addImage(png,'PNG',0,0);
			doc.autoPrint({variant: 'non-conform'});
			//doc.addImage(imgData, 'JPEG', 15, 40, 180, 160)
			//doc.text(20, 20, 'Hello world!');
			
			//SAVE THIS TO AUTO PRINT doc.autoPrint({variant: 'non-conform'});
			$('#embPDFImg').html('<img style="border:1px dashed red" src="'+canvas.toDataURL()+'"> ');
			//doc.save('sample-file.pdf');
			
			var downloaded_pdf = doc.output('blob'); 

			
			var $divs = $('.final_pdf')                
			var numRecursionsNeeded = $divs.length; 
			
			/*
			alert(numRecursionsNeeded)
			var currentRecursion=0;

			function recursiveAddHtmlAndSave(currentRecursion, totalRecursions){
				if(currentRecursion==totalRecursions){
					var downloaded_pdf = doc.output();
					//var p = document.getElementById("printing_status");
					//p.innerHTML = data
					//window.navigator.msSaveOrOpenBlob(fileURL, "Name.pdf");
				} else {
					currentRecursion++;
					doc.addPage();
					doc.addHTML($('.final_pdf')[currentRecursion], 15, 20, options, function(){
			...
			*/
					...
					requestAsync.open("POST",'http://phsacdeanp.cerncd.com/camm/b0783.phsa_cd.cerncd.com/service/PDF_REQUISITION/initialize',true);   
					window.location = "javascript:MPAGES_SVC_AUTH(requestAsync)";  
					requestAsync.send();
					requestAsync.onreadystatechange = function() {
					if (this.readyState == 4 && this.status == 200) {
						var camm_identifier = this.responseText
						var post_url = camm_store_url+'?identifier='+camm_identifier+'&mimeType=application/pdf';
						var get_url = camm_get_url+camm_identifier;
						var attrib_url = camm_attrib_url+camm_identifier;
						//prompt("post_url",post_url)
						cammStore.open("POST",post_url,true);   
						//window.location = "javascript:MPAGES_SVC_AUTH(cammStore)";  
						cammStore.send(downloaded_pdf);
						cammStore.onreadystatechange = function() {
							//alert('cammStore='+this.status)
						if (this.readyState == 4 && this.status == 200) {
							//alert( this.responseText);
							}
						};
						prompt("get_url",get_url)
						//prompt("attrib_url",attrib_url)
						cammGet.open("GET",attrib_url,true);
						window.location = "javascript:MPAGES_SVC_AUTH(cammGet)";
						cammGet.send();
						cammGet.onreadystatechange = function() {
							//alert('cammGet='+this.status)
						if (this.readyState == 4 && this.status == 200) {
								$('#embPDFDiv').html('<iframe style="display:visible" id="iframe" src="'+get_url+'"> ');
							}
						};
					}
					};
				...
				//window.location = "javascript:MPAGES_SVC_AUTH(a)";
				a.send();
			...
			/*requestAsync.open("POST",post_url,true);   
			window.location = "javascript:MPAGES_SVC_AUTH(requestAsync)";  
			requestAsync.send();
			requestAsync.onreadystatechange = function() {
			if (this.readyState == 4 && this.status == 200) {
			...
			$.ajax({
			  ...
			  success: download.bind(true, "application/pdf", "test.pdf")
			});
			*/
			//jsPrintAll();
			
			//embedPDF(PDFurl);
			//$('#embPDFDiv').html('<iframe style="" id="iframe" src="'+PDFurl+'"> ');
		...
		//p.innerHTML = "Loaded " + loadedCount + " of " + loadedCount + " requistions";
		//jsPrintAll();
		
		function jsPrintAll()	{
			var pdf = new jsPDF();	
			var pdfName = 'print.pdf';
			var options = {};
			var $divs = $('.final_pdf')                
			var numRecursionsNeeded = $divs.length;    
			var currentRecursion=0;
			...
			function recursiveAddHtmlAndSave(currentRecursion, totalRecursions){
				if(currentRecursion==totalRecursions){
					...
					currentRecursion++;
					pdf.addPage();
					pdf.addHTML($('.final_pdf')[currentRecursion], 15, 20, options, function(){
						recursiveAddHtmlAndSave(currentRecursion, totalRecursions)
					});
				}
			}
		...
		function embedPDF(PDFurl)	{
			var PDFid = document.getElementById("embPDFDiv");
            var pdfEmbed = document.createElement('embed');
            pdfEmbed.setAttribute('src', PDFurl);
            pdfEmbed.setAttribute('type', 'application/pdf');
            pdfEmbed.setAttribute('width', '0%');
	...
	framecontentElem.on('click', 'span.pwx_fcr_content_type_detail_icon_dt', function (e) {
		...
		task_detailText.push('<th id="thPerformedBy">Performed By</th>');
		task_detailText.push('<th id="thPosition">Position</th>');
		task_detailText.push('<th id="thComment">Validation Column (will be hidden after testing)</th>');
		task_detailText.push('</tr></thead>');
...
turned off auto udpate	
	$("#task_status").on("multiselectclose", function (event, ui) {
		...
		json_task_page_start_numbersAr = [];
		task_list_curpage = 1;
		RenderTaskListContent(pwxstoreddata);
	});
	...
	$("#clerical_status").on("multiselectclose", function (event, ui) {
		framecontentElem.empty();
		framecontentElem.html('<div id="pwx_loading_div"><span class="pwx_loading-spinner"></span><br/><span id="pwx_loading_div_time">0 ' + amb_i18n.SEC + '</span></div>');
		start_pwx_timer()
		start_page_load_timer = new Date();
	...
	$("#task_subtype").on("multiselectclose", function (event, ui) {
		framecontentElem.empty();
		framecontentElem.html('<div id="pwx_loading_div"><span class="pwx_loading-spinner"></span><br/><span id="pwx_loading_div_time">0 ' + amb_i18n.SEC + '</span></div>');
		start_pwx_timer()
		start_page_load_timer = new Date();
	...
	$("#task_priority").on("multiselectclose", function (event, ui) {
		framecontentElem.empty();
		framecontentElem.html('<div id="pwx_loading_div"><span class="pwx_loading-spinner"></span><br/><span id="pwx_loading_div_time">0 ' + amb_i18n.SEC + '</span></div>');
		start_pwx_timer()
		start_page_load_timer = new Date();
