...
function RenderTaskListContent(pwxdata) {
	...
	for (var i = json_task_end_number; i < pwxdata.TLIST.length; i++) {
		...
		var req_subtype_match = 0
		var req_subtype_exist = 0
		...
		if (req_subtype_match == 0) {	
			for (var jj=0; jj < pwxstorefilterdata.GSUBTYPE_LIST.length; jj++) {
				for (var ii = 0; ii < pwxstorefilterdata.GSUBTYPE_LIST[jj].GROUP.length; ii++) {
					if (pwxstorefilterdata.GSUBTYPE_LIST[jj].GROUP[ii].TYPE == pwxdata.TLIST[i].SUB_ACTIVITY_TYPE) {
					req_subtype_exist = 1;
					break;
					}
		...
		//alert(pwxdata.TLIST[i].SUB_ACTIVITY_TYPE+":"+req_subtype_match  +":"+req_subtype_exist )
		if (req_subtype_match == 0 && req_subtype_exist == 0) {
			req_subtype_match  = 1;
			pwxdata.TLIST[i].SUB_ACTIVITY_TYPE = "";
		}

		var req_priority_match = 0
		var req_priority_exist = 0

		for (var cc = 0; cc < pwx_global_priorityArr  .length; cc++) {
			if (pwx_global_priorityArr [cc] == pwxdata.TLIST[i].PRIORITY) {
				req_priority_match = 1;
				break;
			}
		}

		if (req_priority_match == 0) {	
			for (var jj=0; jj < pwxstorefilterdata.PRIORITY_LIST.length; jj++) {
				for (var ii = 0; ii < pwxstorefilterdata.PRIORITY_LIST[jj].GROUP.length; ii++) {
					if (pwxstorefilterdata.PRIORITY_LIST[jj].GROUP[ii].PRIORITY == pwxdata.TLIST[i].PRIORITY) {
					req_priority_exist = 1;
					break;
					}
		...
		if (req_priority_match == 0 && req_priority_exist == 0) {
			req_priority_match = 1;
			pwxdata.TLIST[i].PRIORITY = "";
		}	
		...
		if (status_match == 1 && type_match == 1 && req_subtype_match == 1 && req_priority_match == 1 && cler_status_match == 1) {
			//pwx_task_counter += 1;
